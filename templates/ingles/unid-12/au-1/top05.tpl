{topo}
{tituloAula}
{menu}

<h3><p><a class="" href="#" title="Exercitando">Exercising</a></p></h3>
<div id="center">

<div id="box-esquerda">
<div id="box-atividade">
	<div class="ba-esq">
	    <div class="ba-balao">
	       <p><a class="" href="#" title="Do you know the words to describe a country or city? See the dialogue and then check the translation of the words.">Você sabe as palavras para descrever um país ou cidade? Veja o diálogo e verifique a tradução das palavras.</a></p> 
	    </div>
	    <div class="ba-img"><img src="../../../imgs/e03.png"/></div>
	</div>
</div>
</div>

<div id="box-direita">
<div id="atividade">
		<p><center> Agora vamos fazer a atividade.</center></p>
</div>
</div>




{barra_tools}
	<div id="navegacao">
		<a href="top04.php" class="navegacao-esq"><img src="../../../imgs/seta1.png"/></a>
			<span>Página 5 de 10</span>
		<a href="top06.php" class="navegacao-dir"><img src="../../../imgs/seta2.png"/></a>
	</div>
{rodape}
