{topo}
{tituloAula}
{menu}

<div id="meio">

<h3>Acrescentar Titulo</h3>

<div id="box-center-video">
    <div id="box-dir">
		  <div id="balao-azul">
		    <p>O vídeo ao lado mostra como você pode usar o que aprendeu sobre nacionalidades reforçando seu aprendizado.</p>
		  </div>
		  <div id="person">
		      <img src="../../../imgs/e10.png">
		  </div>
    </div>
    <div id="box-esq">
        <div id="box-video-esq-video">
          http://www.youtube.com/watch?feature=player_embedded&v=qtpKmC1y8WE
         </div>
        <div id="box-video-esq-img">
            <img src="../../../imgs/e11.png">
        </div>
    
    </div>
</div>


</div>	
<div id="rodape">
	<div id="navegacao">
		<a href="top06.php" class="navegacao-esq"><img src="../../../imgs/seta1.png"/></a>
			<span>Página 7 de 10</span>
		<a href="top08.php" class="navegacao-dir"><img src="../../../imgs/seta2.png"/></a>
	</div>
</div>

{rodape}
