{topo}
{tituloAula}
{menu}


<div id="meio">
<h3><a href="#" title="Falando sobre a origem">Talking About Origin</a></h3>

<div id="bos-esquerda">
<div id="box-explicacao-tabela">
    <div id="box-explicacao-tabela-texto">
    <p>Geralmente, quando conhecemos alguém de outro país, queremos saber de onde essa pessoa vem, na tabela estão alguns exemplos de países em inglês</p>
    </div>
    <div id="box-explicacao-tabela-img">
        <img src="../../../imgs/e17.png"/>    
    </div>
</div>
</div>

<div id="box-direita">
<div id="box-explicacao-tabela-left">
	<table>
		<tr>
			<td>United States (Estados Unidos)</td>
            <td> Canada (Canadá) </td>
            
          </tr>
          <tr> 
            <td> Spain (Espanha) </td>
            <td> Argentina (Argentina) </td>
          </tr>  
           <tr> 
            <td> Portugal (Portugal) </td>
            <td> Nigeria (Nigéria) </td>
          </tr>  
           <tr> 
            <td> England (Inglaterra) </td>
            <td>Ghana (Gana) </td>
          </tr> 
           <tr> 
            <td> Italy (Itália) </td>
            <td>Mexico (México) </td>
          </tr>
           <tr> 
            <td> Germany (Alemanha) </td>
            <td>Mexico (México) </td>
          </tr>
          <tr> 
            <td> China (China) </td>
            <td>Japan (Japão) </td>
          </tr>
           <tr> 
            <td> India (Índia) </td>
            <td>Russia (Rússia) </td>
          </tr>
	</table>
</div>
</div>

</div>

{barra_tools}
	<div id="navegacao">
		<a href="top03.php" class="navegacao-esq"><img src="../../../imgs/seta1.png"/></a>
			<span>Página 4 de 10</span>
		<a href="top05.php" class="navegacao-dir"><img src="../../../imgs/seta2.png"/></a>
	</div>
</div>

{rodape}
