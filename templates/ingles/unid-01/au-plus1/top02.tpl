{topo}
{tituloAula}
{menu}

<div id="meio">
<h3><a href="#" class="tooltip-test" title="Revisando!">Brush up!</a></h3>
<div id="center">

<div id="box-esquerda">
<div class="ba-esq">
    <div class="ba-balao">
 		<p>Agora que assistimos ao vídeo, vamos fazer algumas atividades para reforçar o que vimos?</p>
 		<p>Let´s do it!</p>
    </div>
    <div class="ba-img"><img src="../../../imgs/e03.png"/></div>
</div>
</div>

<div id="box-direita">
<div id="box-explicacao-video-animacao">
	<h5><a href="#" class="tooltip-test" title="Complete as frases com as palavras abaixo.">Complete the sentences with the words below.</a></h5>


</div>

</div>
</div>
</div>

{barra_tools}

<div id="navegacao">
<a href="top01.php" class="navegacao-esq"><img src="../../../imgs/seta1.png"/></a>
<span>Página 2 de 9</span>
<a href="top03.php" class="navegacao-dir"><img src="../../../imgs/seta2.png"/></a>

	</div>

{rodape}
