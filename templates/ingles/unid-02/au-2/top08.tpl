{topo}
{menu}

<div id="meio">
<h3>Learn More</h3>

<div id="box-atividade">
<div class="ba-esq">
    <div class="ba-balao">
        <p>Essas palavras são expressões que pode ser usadas em uma conversação. Essa atividade irá lhe auxiliar no aprendizado auditivo e escrito. </p>
        <p>Clique nas palavras e pratique sua pronuncia. </p>
    </div>
    <div class="ba-img"><img src="../../../imgs/e03.png"/></div>
</div>
<div id="explicacao">
	<table width="100%">
		<tbody>
		<tr>
          <td>Hello, Hi</td>
          <td>&nbsp;</td>
          <td>Olá, Oi</td>
        </tr>
        <tr>
          <td>Good morning</td>
          <td>&nbsp;</td>
          <td>Bom dia</td>
        </tr>
        <tr>
          <td>Good night</td>
          <td>&nbsp;</td>
          <td>Boa noite</td>
        </tr>
        <tr>
          <td>Good-bye, bye</td>
          <td>&nbsp;</td>
          <td>Adeus, Tchau</td>
        </tr>
        <tr>
          <td>See you soon</td>
          <td>&nbsp;</td>
          <td>Até logo</td>
        </tr>
        <tr>
          <td>See you later</td>
          <td>&nbsp;</td>
          <td>Até breve</td>
        </tr>
        <tr>
          <td>&nbsp;</td>
          <td>&nbsp;</td>
          <td>&nbsp;</td>
        </tr>
        <tr>
          <td>Please</td>
          <td>&nbsp;</td>
          <td>Por favor</td>
        </tr>
        <tr>
          <td>Thank you</td>
          <td>&nbsp;</td>
          <td>Obrigado</td>
        </tr>
        <tr>
          <td>You're welcome</td>
          <td>&nbsp;</td>
          <td>De nada</td>
        </tr>
        <tr>
          <td>&nbsp;</td>
          <td>&nbsp;</td>
          <td>&nbsp;</td>
        </tr>
        <tr>
          <td>Excuse me</td>
          <td>&nbsp;</td>
          <td>Desculpe, Com licença</td>
        </tr>
        <tr>
          <td>Sorry</td>
          <td>&nbsp;</td>
          <td>Desculpe</td>
        </tr>
        <tr>
          <td>Congratulations</td>
          <td>&nbsp;</td>
          <td>Parabéns</td>
        </tr>
        <tr>
          <td>Good luck</td>
          <td>&nbsp;</td>
          <td>Boa sorte</td>
        </tr>
        <tr>
          <td>&nbsp;</td>
          <td>&nbsp;</td>
          <td>&nbsp;</td>
        </tr>
        <tr>
          <td>Hey</td>
          <td>&nbsp;</td>
          <td>Ei</td>
        </tr>
        <tr>
          <td>What's up?</td>
          <td>&nbsp;</td>
          <td>Como vai?, E aí?</td>
        </tr>
        <tr>
          <td>How are you?</td>
          <td>&nbsp;</td>
          <td>Como vai?, Tudo bom?</td>
        </tr>
        <tr>
          <td>How are you doing?</td>
          <td>&nbsp;</td>
          <td>Como está?</td>
        </tr>
        <tr>
          <td>How's it going?</td>
          <td>&nbsp;</td>
          <td>Como vai?</td>
        </tr>
      </tbody>
		
		</thead>
	</table>
</div>
</div>

</div>	
<div id="rodape">
	<div id="navegacao">
        <a href="top07.php" class="navegacao-esq"><img src="../../../imgs/seta1.png"/></a>  
    	<span>Página 8 de 9</span>
        <a href="top09.php" class="navegacao-dir"><img src="../../../imgs/seta2.png"/></a> 
	</div>
</div>

{rodape}
