{topo}
{tituloAula}
{menu}

<div id="meio">

<h3><a href="#" title="Cosideraçoes iniciais">Initial considerations</a></h3>
<div id="center">

<div id="boxDigitacao">
	    <div class="item">
	      Na lição anterior, aprendemos a expressar nossos desejos e vontades no futuro em frases afirmativas e negativas. Nesta, focaremos em como fazer isso na forma interrogativa.
	    </div>
	</div>

	<div id="boxDialogo">
	<div class="box-digitacao">
	    <div class="areaTexto"></div>
	    <div id="img-person">
	        <img src="../../../imgs/personagem_corpo.png"/>
	    </div>
	</div>
	</div>


</div>
</div>

{barra_tools}

<div id="navegacao">
	<span>Página 1 de 10</span>
	<a href="top02.php" class="navegacao"><img src="../../../imgs/seta2.png"/></a>
</div>
	
{rodape}
