{topo}
{tituloAula}
{menu}

<div id="meio">
<h3><a href="#" title="Conclusão">Conclusion</a></h3>
<div id="center">
	<div id="box-center-conclusao">
	    <div id="box-esq">
	        <div class="box-person">
				<embed width="300" height="450" type="application/x-shockwave-flash" src="../../../swf/s01.swf" wmode="transparent" name="wmode">
	        </div>
	    </div>
	    <div id="box-dir">
		<div id="texto">
		<p>Nesta unidade aprendemos como falar sobre nossos desejos e vontades primeiramente nas forma negativa, já na segunda lição aprendemos de forma interrogativa. Aprendemos também novas expressões e palavras, assim como relembramos verbos já aprendidos. Refletimos um pouco sobre a nossa aprendizagem ao longo do curso.</p>
		<p>Na próxima lição iremos falar sobre nosso Estado!</p>
		<p><span class="label label-info"><a href="#" title="Nos vemos em breve!">See you soon!</span></a></p>

		</div>
	  </div>
</div>

</div>
</div>
</div>
{barra_tools}
<div id="navegacao">
<a href="top04.php" class="navegacao-esq"><img src="../../../imgs/seta1.png"/></a>
	<span>Página 5 de 5</span>
</div>
</div>
{rodape}
