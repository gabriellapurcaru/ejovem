{topo}
{tituloAula}
{menu}

<div id="meio">
<h3><a href="#" title="Conclusão">Conclusion</a></h3>
<div id="center">

	<div id="box-center-conclusao">
	    <div id="box-esq">
	        <div class="box-person">
				<embed width="300" height="450" type="application/x-shockwave-flash" src="../../../swf/s01.swf" wmode="transparent" name="wmode">
	        </div>
	    </div>
	    <div id="box-dir">
		<div id="texto">
		<p>Nessa lição você aprendeu sobre a estrutura de uma pergunta no passado e sobre o verbo auxilixar <b>did</b>. Na próxima lição iremos aprender sobre os verbos irregulares.</p>
<p><span class="label label-info"><a href="" title="Até a próxima aula!">See you next class!</a></span></p>

		</div>
	    </div>
	</div>


</div>		
</div>	
{barra_tools}
<div id="navegacao">
<a href="top11.php" class="navegacao-esq"><img src="../../../imgs/seta1.png"/></a>
	<span>Página 12 de 12</span>
</div>
</div>

{rodape}
